...
class OptimalFrenetPlanning():
    def __init__(self):
        ...
        self.mission_path_pub = rospy.Publisher('/delivery_path_start', Bool, queue_size=2)
        return
    ...
    def callback_path_mode(self, path_mode_msg):
        path_mode_msg.data
    ...
    def callback_obstacles(self, obstacle_msg):
        ...
        # if len(self.obstacles_hx) > 8:
        #     excess_length = len(self.obstacles_hx) - 8
    ...
    def callback_driving_mode(self, mode_msg):
        ...
        # 배달미션(B) => 정상주행으로 넘어갈 때
            ...
            self.driving_mode = mode_msg.data
            self.delivery_pose = None
        # 교차로 -> delivery_finish (속도가 느려짐) -> 배달 B path가 나옴 (더 느려짐) -> 배달구역에서 탈출 ->  Frenet path
    ...
    def get_path(self, path_msg):
        ...
        xs, ys, yaws, s, csp = generate_target_course(xs, ys, step_size=0.5) # 기존 0.2
        path['x'] = xs
        path['y'] = ys
        path['yaw'] = yaws
        path['s'] = s
