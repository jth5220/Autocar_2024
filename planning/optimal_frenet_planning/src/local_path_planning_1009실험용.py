...
class OptimalFrenetPlanning():
    def __init__(self):
        ...
        # ROS
        rospy.init_node('optimal_frenet_planning')
        ...
        self.mission_pose_sub = rospy.Subscriber('/mission_pose', PoseStamped, self.callback_mission_pose)
    ...
    # 실제 주행할때 바꿔야함
    def callback_mission_pose(self, pose_msg):
        self.delivery_pose = [pose_msg.pose.position.x, pose_msg.pose.position.y]
    ...
    def callback_driving_mode(self, mode_msg):
        ...
        #     print("주차 모드로 전환")
        #     self.path_finder = Parking(self.ref_path, self.car_pose, min_R = 4.0)
        #     self.driving_mode = mode_msg.data

        if self.driving_mode == 'normal_driving' and mode_msg.data == 'parking':
            print("주차 모드로 전환")
            self.path_finder = Parking(self.ref_path, self.car_pose, min_R = 4.0)
            self.driving_mode = mode_msg.data
    ...
    def get_path(self, path_msg):
        ...
        xs, ys, yaws, s, csp = generate_target_course(xs, ys, step_size=0.5)
        path['x'] = xs
        path['y'] = ys
        path['yaw'] = yaws
        path['s'] = s
