...
class Delivery():
    def __init__(self, ref_path, car_pose_init, delivery_spot, min_R, delivery_mode):
        ...
        self.min_R1 = min_R1 # entry중 시작지점에 가까운 원
        self.min_R2 = min_R2 # entry중 목표지점에 가까운 원
        self.min_R3 = min_R3 # exit중 시작지점에 가까운 원
        self.min_R4 = min_R4 # exit중 목표지점에 가까운 원
    ...
    def find_delivery_path(self, car_pose, delivery_spot_utm):
        ...
        if self.delivery_mode == "delivery_A":
            self.spot_adjustment_x = 0
            self.spot_adjustment_y = 1.3

            delivery_spot[0] += self.spot_adjustment_x
            delivery_spot[1] += self.spot_adjustment_y

            # 2. 회전시킨 delivery spot에서 1.0m 아래으로 이동 매끄러운 진입을 위해 목표지점을 내림
            delivery_margin = 5.0
            delivery_spot[0] -= delivery_margin #아래
            
            # 3. 원2의 중심과 반지름
            radius2 = self.min_R2 # 최소회전반경
            ...
            radius3 = self.min_R3
            center3 = (delivery_spot[0] + delivery_margin ,delivery_spot[1] + radius3)
            ...
            delta_x = np.sqrt(distance**2 - delta_y**2)      
            x4 = delivery_spot[0] + delivery_margin + delta_x 
            center4 = (x4, y4)
            ...
            p4 = (delivery_spot[0] + delivery_margin, delivery_spot[1])
            ...
            self.spot_adjustment_x = -1.0
            self.spot_adjustment_y = 0
            ...
            # 5. 원3의 중심과 반지름(배달 구역에서 나가기 위한 원) -> 만약 배달 구역이 좀 좁다면 min_R을 줄여야함
            radius3 = self.min_R3
            center3 = (delivery_spot[0], delivery_spot[1] + radius3)
            ...
            p4 = (delivery_spot[0] + delivery_margin, delivery_spot[1]) # 이 점이 가장 최종 주차 위치
            p5 = ((center3[0]+center4[0])/2, (center3[1]+center4[1])/2)
            p6 = (center4[0],0)
            ...
            rotate_path4_x, rotate_path4_y = create_straight_path(p3, p4, step_size)
            ...
            start_angle1 = np.arctan2(p3[1] - center3[1], p3[0] - center3[0])
            end_angle1 = np.arctan2(p5[1] - center3[1], p5[0] - center3[0])
            p_x3, p_y3 = points_on_circle(center3, radius3, start_angle1, end_angle1)
