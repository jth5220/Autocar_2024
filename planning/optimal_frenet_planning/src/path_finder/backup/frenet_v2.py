...
class Frenet(object):
    def __init__(self, ref_path, car_pose, robot_radius, lane_width, possible_change_direction):
        ...
        if possible_change_direction == 'none':
            # cost weights
            ...
            # self.DF_SET = np.array([ -self.LANE_WIDTH * 0.25, -self.LANE_WIDTH * 0.2, 0.0, 
            #                           self.LANE_WIDTH * 0.25, self.LANE_WIDTH * 0.3, self.LANE_WIDTH * 0.35, self.LANE_WIDTH * 0.40])
            self.DF_SET = np.array([-self.LANE_WIDTH * 0.28, 0.0, 
                                      self.LANE_WIDTH * 0.25, self.LANE_WIDTH * 0.3, self.LANE_WIDTH * 0.35, self.LANE_WIDTH * 0.40])
            ...
            # self.DF_SET = np.array([-self.LANE_WIDTH * 0.35, -self.LANE_WIDTH * 0.3, -self.LANE_WIDTH * 0.275, -self.LANE_WIDTH * 0.25, -self.LANE_WIDTH * 0.2, 0.0, 
            #                           self.LANE_WIDTH * 0.2, self.LANE_WIDTH * 0.25, self.LANE_WIDTH * 0.3, self.LANE_WIDTH * 0.35])

            # self.MAX_SF = 12.0 # 길이
            # self.MIN_SF = 4.0 # 최소 회전 반경
            # self.DS = 6.0

        else:
            # cost weights
            self.K = 10 # 원래 ref path로 돌아오려는 weight
            self.K_DIFF = 3
            self.K_KAPPA = 0
            self.K_MEAN_S = 0
            self.K_SHORT = 0
            ...
            if possible_change_direction == 'right':
                ...
                self.DF_SET = np.array([ 0.0, self.LANE_WIDTH * 0.8, self.LANE_WIDTH*1.2])
                # self.DF_SET = np.array([-0.3 * self.LANE_WIDTH, 0.0, self.LANE_WIDTH * 0.8, self.LANE_WIDTH*1.2])
            else: # possible_change_direction == 'both':
